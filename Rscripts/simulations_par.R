...
#
# e.g.: R --vanilla --args S 1-100 50 Rscripts/sim_par_example.R ~/scratch/data/Symbiosis_theory/example_sim < Rscripts/simulations_par.R
...
source("Rscripts/sim_functions.R")
...
for(g in 1:gens){
	...
	sp1 = lapply(sp1, mate, K1, rec1, mu1)
	sp2 = lapply(sp2, mate, K2, rec2, mu2)
	...
	# selection
	if(intype=="S") survs = mapply(function(x,y) selS(x,y,om,C1,B1,C2,B2), sp1, sp2)
	if(intype=="R") survs = mapply(function(x,y) selR(x,y,C1,B1,C2,B2), sp1, sp2)
	if(intype=="SR") survs = mapply(function(x,y) selR(x,y,C1,B1,C2,B2), sp1, sp2)

	if(intype!="N"){ sp1 = survs[1,]; sp2 = survs[2,] }
...
clusterExport(cl, c("Sim", "intype", "gens", "reps", "Npops", "K1s", "K2s", "m1s", "m2s", "p11s", "p12s", "p21s", "p22s", "rec1s", "rec2s", "mu1s", "mu2s", "C1s", "B1s", "C2s", "B2s", "oms"))
